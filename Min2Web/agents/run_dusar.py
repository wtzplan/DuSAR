...
from agents.DuSTA import eval_traj_sample,eval_auto_sample
...
def create_parser():
    ...
    parser.add_argument("--with_double", action="store_true", default=False)
    parser.add_argument("--with_mark", action="store_true", default=False)
    parser.add_argument("--with_action", action="store_true", default=False)
    parser.add_argument("--with_global", action="store_true", default=False)
    parser.add_argument("--with_local", action="store_true", default=False)
...
def main():
    ...
    with tqdm(total=n) as t:
        for i in range(147, n, args.chunk_size):
            chunk = samples[i : min(i + args.chunk_size, n)]
            with ThreadPoolExecutor() as executor:
                if args.with_double:
                    iterator = executor.map(
                    lambda p: eval_auto_sample(p[0], args, p[1], memory, memory_mapping, thoughts),
                    zip(range(i + start_idx, i + start_idx + len(chunk)), chunk),
                )
                    #eval_auto_sample(p[0], args, p[1], memory, memory_mapping, thoughts)
                else:
                    iterator = executor.map(
                    lambda p: eval_traj_sample(p[0], args, p[1], memory, memory_mapping, thoughts),
                    zip(range(i + start_idx, i + start_idx + len(chunk)), chunk),
                )
                    #eval_traj_sample(p[0], args, p[1], memory, memory_mapping, thoughts)
    ...
    #         # 构造参数列表
    #         params = zip(range(i + start_idx, i + start_idx + len(chunk)), chunk)
